...
function base_url()
{
	return 'https://localhost/dev/Library Management System';
}
...
{
	$query = "
	SELECT library_timezone FROM setting 
	LIMIT 1
	";
...
function get_one_day_fines($connect)
{
	$output = 0;
	$query = "
	SELECT library_one_day_fine FROM setting 
	LIMIT 1
	";
	$result = $connect->query($query);
	foreach($result as $row)
...
function get_currency_symbol($connect)
{
	$output = '';
	$query = "
	SELECT library_currency FROM setting 
	LIMIT 1
	";
	$result = $connect->query($query);
	foreach($result as $row)
...
function get_book_issue_limit_per_user($connect)
{
	$output = '';
	$query = "
	SELECT library_issue_total_book_per_user FROM setting 
	LIMIT 1
	";
	$result = $connect->query($query);
	foreach($result as $row)
...
function get_total_book_issue_day($connect)
{
	...
	$query = "
	SELECT library_total_book_issue_day FROM setting 
	LIMIT 1
	";
...
{
	$query = "
	SELECT location_rack_name FROM location_rack 
